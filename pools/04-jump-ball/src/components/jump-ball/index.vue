...
const list = Array.from({ length: 150 }, (_, i) => {
  ...
  const baseWidth = window.screen.width > 800 ? 200 : 70
  return {
    delay: `-${19973 + i * 200}ms`,
    duration: 7000 + i * 10,
    left: `${baseWidth + random(0, 100)}`,
        ...
        <div class="camera">
          <div class="particles">
            <div class="particle" v-for="(item, index) in list" :key="index">
              <div class="rotate"
                :style="`animation-delay:${item.delay};animation-duration:${item.duration}ms;--delay:${item.delay};`">
                <div class="part" :style="`left:calc(50% + ${item.left}px);`">
                  <div class="rotatefollow"
                    :style="`animation-delay:${item.delay};animation-duration:${item.duration}ms;`">
                    <div class="camerafollow">
                      <div class="camerafollow">
                        <div class="camerafollow x">
                          <div class="graphic"
                            :style="`background:${item.background};animation-delay:${item.delay};--before-bg:${item.beforeBg};`">
                          </div>
                        </div>
                      </div>
                    </div>
